// npm i discord.js dotenv
require("dotenv").config();
const {
  Client,
  GatewayIntentBits,
  PermissionsBitField,
  ChannelType,
} = require("discord.js");

const CONFIG = {
  guildId: process.env.GUILD_ID,
  ownerUserId: process.env.OWNER_USER_ID || null,
  createRoles: ["Ops Team", "Project Core", "Contributors"],

  categories: {
    "Agent Direct Lines": ["henry", "violet", "quill", "pixel", "echo", "charlie", "charlie-logs"],
    "Projects": ["agent-dashboard", "skool-ai-extension", "agent-org-infra", "micro-saas-factory", "content"],
    "Ops": ["announcements", "runbook", "incidents"],
    "Logs": ["agent-events", "system-alerts", "audit-trail"],
  },
};

const client = new Client({ intents: [GatewayIntentBits.Guilds] });

async function ensureRole(guild, name) {
  let role = guild.roles.cache.find(r => r.name === name);
  if (!role) role = await guild.roles.create({ name, mentionable: true, reason: "Bootstrap role" });
  return role;
}

async function ensureCategory(guild, name, overwrites = []) {
  let cat = guild.channels.cache.find(c => c.type === ChannelType.GuildCategory && c.name === name);
  if (!cat) {
    cat = await guild.channels.create({ name, type: ChannelType.GuildCategory, permissionOverwrites: overwrites });
  } else if (overwrites.length) {
    await cat.permissionOverwrites.set(overwrites);
  }
  return cat;
}

async function ensureTextChannel(guild, name, parentId, overwrites) {
  let ch = guild.channels.cache.find(
    c => c.type === ChannelType.GuildText && c.name === name && c.parentId === parentId
  );
  if (!ch) {
    ch = await guild.channels.create({
      name,
      type: ChannelType.GuildText,
      parent: parentId,
      permissionOverwrites: overwrites,
    });
  } else {
    await ch.permissionOverwrites.set(overwrites);
  }
  return ch;
}

function assertEnv() {
  const missing = ["BOT_TOKEN", "GUILD_ID"].filter(k => !process.env[k]);
  if (missing.length) {
    throw new Error(`Missing required env vars: ${missing.join(", ")}. Copy .env.example to .env and fill values.`);
  }
}

client.once("ready", async () => {
  try {
    assertEnv();

    const guild = await client.guilds.fetch(CONFIG.guildId);
    await guild.roles.fetch();
    await guild.channels.fetch();

    const me = await guild.members.fetchMe();
    const botRoleId = me.roles.botRole?.id || me.roles.highest.id;
    const everyoneId = guild.roles.everyone.id;

    if (!CONFIG.ownerUserId) {
      const owner = await guild.fetchOwner();
      CONFIG.ownerUserId = owner.id;
      console.log(`ℹ️ OWNER_USER_ID not set; defaulting to guild owner: ${CONFIG.ownerUserId}`);
    }

    const roles = {};
    for (const roleName of CONFIG.createRoles) {
      roles[roleName] = await ensureRole(guild, roleName);
    }

    const ownerAllow = [
      PermissionsBitField.Flags.ViewChannel,
      PermissionsBitField.Flags.SendMessages,
      PermissionsBitField.Flags.ReadMessageHistory,
      PermissionsBitField.Flags.ManageMessages,
    ];

    const botAllow = [
      PermissionsBitField.Flags.ViewChannel,
      PermissionsBitField.Flags.SendMessages,
      PermissionsBitField.Flags.ReadMessageHistory,
      PermissionsBitField.Flags.ManageMessages,
    ];

    for (const [categoryName, channels] of Object.entries(CONFIG.categories)) {
      const categoryOverwrites = [
        { id: everyoneId, deny: [PermissionsBitField.Flags.ViewChannel] },
        {
          id: CONFIG.ownerUserId,
          allow: [PermissionsBitField.Flags.ViewChannel, PermissionsBitField.Flags.ManageChannels, PermissionsBitField.Flags.ManageMessages],
        },
        {
          id: botRoleId,
          allow: [PermissionsBitField.Flags.ViewChannel, PermissionsBitField.Flags.ManageChannels, PermissionsBitField.Flags.ManageMessages],
        },
      ];

      const category = await ensureCategory(guild, categoryName, categoryOverwrites);

      for (const chName of channels) {
        let overwrites = [];

        if (categoryName === "Agent Direct Lines") {
          overwrites = [
            { id: everyoneId, deny: [PermissionsBitField.Flags.ViewChannel] },
            { id: CONFIG.ownerUserId, allow: ownerAllow },
            { id: botRoleId, allow: botAllow },
          ];
        } else if (categoryName === "Projects") {
          overwrites = [
            { id: everyoneId, deny: [PermissionsBitField.Flags.ViewChannel] },
            { id: CONFIG.ownerUserId, allow: ownerAllow },
            {
              id: roles["Project Core"].id,
              allow: [
                PermissionsBitField.Flags.ViewChannel,
                PermissionsBitField.Flags.SendMessages,
                PermissionsBitField.Flags.ReadMessageHistory,
              ],
            },
            {
              id: roles["Contributors"].id,
              allow: [
                PermissionsBitField.Flags.ViewChannel,
                PermissionsBitField.Flags.SendMessages,
                PermissionsBitField.Flags.ReadMessageHistory,
              ],
            },
            { id: botRoleId, allow: botAllow },
          ];
        } else if (categoryName === "Ops") {
          overwrites = [
            { id: everyoneId, deny: [PermissionsBitField.Flags.ViewChannel] },
            { id: CONFIG.ownerUserId, allow: ownerAllow },
            {
              id: roles["Ops Team"].id,
              allow: [
                PermissionsBitField.Flags.ViewChannel,
                PermissionsBitField.Flags.SendMessages,
                PermissionsBitField.Flags.ReadMessageHistory,
              ],
            },
            { id: botRoleId, allow: botAllow },
          ];
        } else if (categoryName === "Logs") {
          overwrites = [
            { id: everyoneId, deny: [PermissionsBitField.Flags.ViewChannel] },
            {
              id: CONFIG.ownerUserId,
              allow: [
                PermissionsBitField.Flags.ViewChannel,
                PermissionsBitField.Flags.ReadMessageHistory,
              ],
            },
            {
              id: roles["Ops Team"].id,
              allow: [
                PermissionsBitField.Flags.ViewChannel,
                PermissionsBitField.Flags.ReadMessageHistory,
              ],
              deny: [PermissionsBitField.Flags.SendMessages],
            },
            {
              id: botRoleId,
              allow: [
                PermissionsBitField.Flags.ViewChannel,
                PermissionsBitField.Flags.SendMessages,
                PermissionsBitField.Flags.ReadMessageHistory,
              ],
            },
          ];
        }

        await ensureTextChannel(guild, chName, category.id, overwrites);
      }
    }

    console.log("✅ v2 bootstrap complete: categories, channels, roles, permissions.");
    console.log("Roles created/verified:", Object.keys(roles).join(", "));
  } catch (err) {
    console.error("❌ Bootstrap error:", err);
    process.exitCode = 1;
  } finally {
    client.destroy();
  }
});

client.login(process.env.BOT_TOKEN);
